...
class CouchDB:
    ...
    def checkDataBases(self):
        ...
        if not isDBmd:
            DBmd = self.service.put_database(db='dbmd', partitioned=False).get_result()
            logger.debug(f"Створено БД {DBmd}")
            if DBmd['ok']:
                logger.debug(f"Створено БД: dbmd створена успішно! ")
            else:
                logger.debug(f"Створено БД: dbmd НЕ створено !!!! ")   
    ...
    def saveImage(self, img, imgprops):
        ...
        imgdsc["_id"]=uuid
        # {"filename": element_image_filename, "filedsc": "Picture", "correlation_id":  msgo.get('document_id'), "contenttype": "iamge/png"}
        imgdsc["filename"]=imgprops["filename"]
        imgdsc["description"]=imgprops["filedsc"]
        imgdsc["correlation_id"]=imgprops["correlation_id"]
        imgdsc["contenttype"]=imgprops["contenttype"]
    ...
    def addMarkDownDocumet(self, corelation_id, filename) :
        """
            Створити метадані документа з markdown

        """

        doc={}
        #uuid=None
        #uuidresponse = self.service.get_uuids(count=1).get_result()
        #uuid=uuidresponse["uuids"][0]

        doc["_id"]=corelation_id
        doc["typedoc"]="MARKDOWN"
        doc["filename"]=filename
        doc["correlation_id"]=corelation_id
        doc["content_type"]="text/markdown"
  
        docresponse = self.service.post_document(db='dbmd', document=doc).get_result()  
    ...
    def addMarkDownAttachment(self, docid, attachname, attach):
        """
          Додати документ з markdown як attachment
        """
        doc = self.service.get_document(db='dbmd',doc_id=docid).get_result()
        rev=doc["_rev"]
        r = self.service.put_attachment(db='dbmd',doc_id=docid, attachment_name=attachname, attachment=attach, content_type='text/markdown', rev=rev )
        return r.get_result()

    def isMarkDownExists(self,docid):
        """
          Чи вже є такий документа в БД?
        """
        try:
            doc = self.service.head_document(db='dbmd',doc_id=docid)
            return {"ok": True}
